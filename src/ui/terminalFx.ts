...
import { themes, themeStore } from './theme';

/**
 * The CRT themes' screen effects — the part of the look that is behaviour
 * rather than palette, so the part a reader may want off while keeping the
 * phosphor: a scanline overlay on every screen, and question prompts that
 * type themselves out under a block cursor.
 *
 ...
 * scanlines by accident: it keys on the theme's `crt` FLAG, not its name, so
 * every phosphor theme (Terminal's green, Amber's amber) gets the full rig
 * and everything else is constant `false`.
 */
export function useCrtScreen(): boolean {
  const active = themes[themeStore.use()].crt;
